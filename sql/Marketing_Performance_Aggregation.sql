...
-- ============================================================================
-- SECTION 3: DATA AGGREGATION & BUSINESS LOGIC (FINAL QUERY)
-- ============================================================================
...
SELECT 
    ...
    SUM(CASE WHEN m.is_purchased = 1 AND (m.`date` <= fp.first_purchase_date OR fp.first_purchase_date IS NULL) THEN 1 ELSE 0 END) AS new_customers_acquired,
    SUM(CASE WHEN m.is_purchased = 1 AND m.`date` > fp.first_purchase_date THEN 1 ELSE 0 END) AS existing_customers_reengaged
FROM messages_demo m
INNER JOIN campaigns c 
    ON m.campaign_id = c.id
LEFT JOIN client_first_purchase fp 
    ON m.client_id = fp.client_id
LEFT JOIN holidays h 
    ON m.`date` = h.holiday_date
WHERE (c.is_test = 0 OR c.is_test IS NULL)
GROUP BY m.campaign_id, c.campaign_type, c.channel, m.channel, c.topic, m.`date`, h.holiday_date, h.holiday_name;
